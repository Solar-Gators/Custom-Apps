...
4. **Save file** — one `.json` holding the data, the drawing and its picture.
5. **Open file** picks it back up on any computer. Nothing is uploaded; there is no account and no server. USE THE JSON FILE ABOVE TO SEE CAR 5 SO FAR.
6. **Export finalised** — one read-only `.html` of the finished diagram. It opens in any browser, still shows what every wire joins, and cannot be edited.
